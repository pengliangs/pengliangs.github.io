...
&emsp;近期公司OA系统即将到期，公司决定根据自己的业务做一套OA系统。在起初的架构选型中采用的是纯HTML 由于人员变动前台小哥哥单飞了；项目就由我接手了 恰巧前段时间刚学习了Vue对它也是情有独钟就提出采用Vue全家桶来构建一个单页面应用，在开发过程中也是磕磕碰碰遇到了不少的问题在此进行总结记录；
